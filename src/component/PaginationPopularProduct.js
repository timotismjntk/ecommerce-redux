...
const Paginations = ({ postsPerPage, totalPosts, currentPage, pageInfo }) => {
  ...
  const slideToPage = (number) => {
      ...
      dispatch(popularProductAction.getPopularProduct(number))
  }
  ...
  const nextPage = () => {
    if (pageInfo.nextLink) {
      ...
      dispatch(popularProductAction.getPopularProduct(pageInfo.currentPage + 1))
    }
  }
  const prevPage = () => {
...
/* ----------------------------------------------------------------- */
  ...
  return (
      <div className="w-100">
        ...
        <PaginationItem onClick={() => {slideToFirstPage(pageInfo.currentPage - First + 1)}} className={`${ FirstStyle }`}>
          <PaginationLink first/>
        </PaginationItem >
        <PaginationItem onClick={() => {prevPage()}} className={`${ PrevStyle }`}>
          <PaginationLink previous/>
        </PaginationItem>
        {pageNumbers.map(number => (
            <PaginationItem className={`page-item${ pageInfo.currentPage === number ? ' active' : ''}`}>     {/* untuk menunjukan page yang sedang aktif  */}
            <PaginationLink onClick={() => {slideToPage(number)}}>
                {number}
            </PaginationLink >
            </PaginationItem>
        ))}
          <PaginationItem onClick={() => {nextPage()}} className={`${ NextStyle }`}>
            <PaginationLink next/>
          </PaginationItem>
          <PaginationItem onClick={() => {slideToLastPage(pageInfo.currentPage + Last)}} className={`${ LastStyle }`}>
            <PaginationLink last/>
          </PaginationItem>
        </Pagination >
      </div>
