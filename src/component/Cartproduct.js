...
export default function Cart(props) {
    ...
    const quantityState = useSelector(state=>state.cart)
    const {info, isError, isLoading, isSelected} = quantityState
    useEffect(()=>{
        if(isLoading) {
            dispatch(cartAction.getCart(token))
        }
    ...
    // }, [dispatch, isLoading, isError, token, info.length])
    ...
    return (
        <Container>
            ...
            <div className="Cart d-flex flex-row">
                ...
                {info.length > 0 ? Object.keys(info) && Object.length && info.map(items=>{
                    return (
                ...
                <div className="item-summary">
                    <h4>Shopping summary</h4>
                        <div className="total-price d-flex flex-row justify-content-between"> 
                            <p>Total price</p>
                            <h5 className="harga">{totalPrice}</h5>
                        </div>
